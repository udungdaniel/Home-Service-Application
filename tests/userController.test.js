...
describe('User Controller - Unit Tests (GET Routes)', () => {
    ...
    beforeEach(() => {
        // Reset mock request and response onjects for each test
        req = {
            params: {}
        };
        res = {
    ...
    describe('getAllUsers(GetAll Route)', () => {
        it('should return 200 and a list of all users on success', async () => {
            const mockUsers = [
                { _id: '6a281865f1fab9b589746921', name: 'Daniel', email: 'danielalfred@gmail.com' },
                { _id: '6a281e46f1fab9b589746924', name: 'Udung', email: 'daniel@gmail.com' }
            ];
    ...
    describe('getUserById (GetRoute)', () => {
        ...
        it('should return 404 if the user is not found in the database', async () => {
            ...
            it('should return 200 and the user object on success', async () => {
                const validMockId = new mongoose.Types.ObjectId().toString();
                req.params.id = validMockId;
                const mockUser = { _id: validMockId, name: 'Daniel', email: 'danielalfred@gmail.com' };

                jest.spyOn(User, 'findById').mockResolvedValue(mockUser);

                await getUserById(req, res);

                expect(Usert.findById).toHaveBeenCalledWith(validMockId);
                expect(res.status).toHaveBeenCalledWith(200);
                expect(res.json).toHaveBeenCalledWith({
                    success: true,
                    data: mockUser
